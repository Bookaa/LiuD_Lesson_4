...
import lesson3_2 as GDL
from lesson3_2 import GDL02_Parser

g_prefix = 'LiuD'
g_skipspace = 'skipspace'
...
class gen_common:
    ...
    def get_type(self,node):
        if isinstance(node, GDL.GDL02_litstring):
            return type_none
        if isinstance(node, GDL.GDL02_litname):
            name = node.s
            return self.get_type0(name)
        if isinstance(node, GDL.GDL02_value1):
            return self.get_type(node.v)
        if isinstance(node, GDL.GDL02_enclosed):
            return self.get_type(node.v)
        if isinstance(node, GDL.GDL02_value):
            return self.get_type(node.v)
        if isinstance(node, GDL.GDL02_itemd):
            t = self.get_type(node.v)
            return self.tolst(t)
        if isinstance(node, GDL.GDL02_series):
            lst = []
            for v1 in node.vlst:
                t = self.get_type(v1)
                if t == type_none:
            ...
            assert False
            return type_v
        if isinstance(node, GDL.GDL02_stmt_value):
            lst = self.get_types(node)
            assert len(lst) == 1
            return lst[0]
        assert False
    def get_types(self,node):
        assert isinstance(node, GDL.GDL02_stmt_value)
        lst = []
        v = node.v
        if isinstance(v, GDL.GDL02_series):
            for v1 in v.vlst:
                t = self.get_type(v1)
                if t == type_none:
                    continue
                lst.append(t)
            return lst
        if isinstance(v, GDL.GDL02_values_or):
            typ = None
            for name in v.slst:
                ...
                if typ is None:
                    ...
                    assert typ == typ1
            return [typ]
        if isinstance(v, GDL.GDL02_string_or):
            return [type_s]
        if isinstance(v, GDL.GDL02_jiap):
...
'''
        ...
        argstr = ', '.join(prefix)
        self.outtxt += txt % (g_prefix, name, argstr)
        for s in prefix:
...
''' % name
        ...
        self.outp('return %s_%s(%s)' % (g_prefix, name, ', '.join(prefix)))
        self.curtyp = None
    ...
    def visit_itemd(self, node):
        if self.curpos == pos_onlyme:
            ...
            v1 = DirectToV(node.v)
            if isinstance(v1, GDL.GDL02_litname):
                name = v1.s
                if name in self.itemlst:
    ...
    def inloop(self, node, lstname):
        ...
        if isinstance(node1, GDL.GDL02_series):
            vname = None
            for i,v in enumerate(node1.vlst):
                if i != 0:
                    self.skipspace()
                v = DirectToV(v)
                if isinstance(v, GDL.GDL02_litname):
                    name = v.s
                    if name in self.itemlst:
                        self.outp('v = self.handle_%s()' % name)
                        self.outp('if not v:')
            ...
            self.skipspace()
            return
        if isinstance(node1, GDL.GDL02_litname):
            name = node1.s
            if name in self.itemlst:
                self.outp('v = self.handle_%s()' % name)
                self.outp('if not v:')
...
def Gen_All(txt):
    the = GDL02_Parser(txt)
    mod = the.handle_main()

    s1 = Gen01(mod)
    s2 = Gen02(mod)

    s = '''# auto generated,  LiuD_LessonFour
...
class Test(unittest.TestCase):
    ...
    def test2(self):
        global g_prefix, g_skipspace
        g_prefix = 'C'
        g_skipspace = 'skipspacecrlf'
